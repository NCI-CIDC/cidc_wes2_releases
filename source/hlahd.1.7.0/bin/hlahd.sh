...
#Drop non-exon read pair
DSE1=${OD}/${ID}/mapfile/${ID}.all_exon.R1.D.sam
DSE2=${OD}/${ID}/mapfile/${ID}.all_exon.R2.D.sam
DSI1=${OD}/${ID}/mapfile/${ID}.all_intron.R1.D.sam
DSI2=${OD}/${ID}/mapfile/${ID}.all_intron.R2.D.sam

${BW} -x ${HD}/all_exon_N${NLEN}.fasta -U ${F1} -S ${DSE1}
${BW} -x ${HD}/all_exon_N${NLEN}.fasta -U ${F2} -S ${DSE2}
${BW} -x ${HD}/all_intron_N${NLEN}.fasta -U ${F1} -S ${DSI1}
${BW} -x ${HD}/all_intron_N${NLEN}.fasta -U ${F2} -S ${DSI2}

RE=${OD}/${ID}/mapfile/${ID}.exon.over
RE1=${OD}/${ID}/mapfile/${ID}.exon.over.R1.fastq
RE2=${OD}/${ID}/mapfile/${ID}.exon.over.R2.fastq
...
zcat ${F1} > ${TMPF1}
zcat ${F2} > ${TMPF2}
drop_intron_map ${DSE1} ${DSE2} ${DSI1} ${DSI2} ${TMPF1} ${TMPF2} ${RE}
rm ${TMPF1}
rm ${TMPF2}
else
drop_intron_map ${DSE1} ${DSE2} ${DSI1} ${DSI2} ${F1} ${F2} ${RE}
fi
...
${BW} -x ${HD}/all_exon_intron_N${NLEN}.fasta -U ${RE1} -S ${ASAM1}
${BW} -x ${HD}/all_exon_intron_N${NLEN}.fasta -U ${RE2} -S ${ASAM2}
R1=${OD}/${ID}/mapfile/${ID}.R1.fastq
R2=${OD}/${ID}/mapfile/${ID}.R2.fastq

${ST} -L ${TSIZE} ${ASAM1} ${RE1} ${OD}/${ID}/mapfile/${ID}
mv ${OD}/${ID}/mapfile/${ID}.fastq ${R1}
${ST} -L ${TSIZE} ${ASAM2} ${RE2} ${OD}/${ID}/mapfile/${ID}
