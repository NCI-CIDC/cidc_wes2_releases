...
def returnCode(log, sample='', process='', outfile=''):
    ...
    log_str = 'echo "[init-workflow] ' + t + '- INFO - Return Code for ' + sample + ' ' + process + ': $? ">> ' + log
...
def hisatOutput(ends, saveunmapped=False):
    res = {'aln': snakemake.io.temp('tmp/{sample}.sam')}

    if(saveunmapped):
        res['un_aln'] = expand('bam/{{sample}}_unmapped_{pe}.fastq.gz', pe=ends)
    else:
        res['un_aln'] = snakemake.io.temp(expand('bam/{{sample}}_unmapped_{pe}.fastq.gz', pe=ends))
...
def sortBamOutput(temp, iontorrent=0, cram=0):
    if iontorrent:
